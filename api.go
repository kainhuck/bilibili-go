...
// GetDeliveryList 查询用户投稿视频列表 https://api.bilibili.com/x/space/arc/search
// mid 目标用户id
// order 排序方式
//
//	最新发布：pubdate
...
//	留空则为pubdate
//
// tid 筛选目标分区，0 不进行分区 分区ID为所筛选的分区 默认0
// keyword 用于使用关键字搜索UP主视频稿件，留空则不筛选
// pn 页码
// ps 每页项数
func (c *Client) GetDeliveryList(mid interface{}, order string, tid int, keyword string, pn int, ps int) (*GetDeliveryListResponse, error) {
	...
	err := c.getHttpClient(true).Get(uri).
		...
		AddParams("tid", strconv.Itoa(tid)).
